"""Coordinator-derived sensor definitions for Elisa Kotiakku."""
...
class ElisaKotiakkuCoordinatorSensor(ElisaKotiakkuEntity, SensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, str | int | float | bool] | None:
        ...
        if key in _ECONOMICS_SENSOR_KEYS:
            economics_last_period_end = (
                self.coordinator.get_economics_last_period_end()
            )
            if economics_last_period_end is None:
                return None
            attrs["last_period_end"] = economics_last_period_end
            ...
            if key in {"total_battery_savings", "skipped_savings_windows"}:
                attrs["skipped_windows"] = (
                    self.coordinator.get_skipped_savings_window_count()
                )
            ...
            if key == "total_battery_house_supply_value":
                attrs["interpretation"] = "gross_avoided_import_not_net_savings"
            ...
            if key == "monthly_first_day_of_profit":
                ...
                attrs["akkureservihyvitys_eur"] = self.coordinator.akkureservihyvitys
                attrs["basis"] = "linear_interpolation"

            if key == "payback_remaining_months":
                attrs["total_battery_cost_eur"] = self.coordinator.battery_total_cost
                total_savings = (
                    self.coordinator.get_economics_total("battery_savings") or 0.0
                )
                tracked_months = len(
                    self.coordinator.get_monthly_battery_savings()
                )
